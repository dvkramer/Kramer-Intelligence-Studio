    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                console.warn("Could not parse API error JSON.");
                try {
            ...
            if (response.status === 413) {
                errorMsg = `Request Failed: Payload too large (${response.statusText}). Max file size is ~${MAX_FILE_SIZE_MB}MB.`;
            }
                ...
                if (line.startsWith('data: ')) {
                    ...
                    try {
                        ...
                        if (event.type === 'status') {
                            showLoading(event.data.status);
                        } else if (event.type === 'result') {
                            const { text, searchSuggestionHtml, modelUsed } = event.data;
                            ...
                            conversationHistory.push({ role: 'model', parts: [{ text }], id: aiMessageId });
                            displayMessage('ai', text, null, searchSuggestionHtml, aiMessageId);
                        } else if (event.type === 'error') {
                            throw new Error(event.data.message || 'An unknown error occurred on the server.');
                        }
                    } catch (e) {
